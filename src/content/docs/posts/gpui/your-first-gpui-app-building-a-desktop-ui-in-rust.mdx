...
By the end of this article, we will have this kind of UI:
![Final look of the GPUI issue tracker UI](/images/gpui/first/final-result.png)
...
- Have Rust installed on your machine ([installation guide](https://www.rust-lang.org/tools/install))
...
> As GPUI is evolving, you may encounter some breaking changes in future versions. If you do, then please inform me so that I can update the article accordingly. You can either go to the blog's Github repo's issues tab and create a new issue or inform me through any social media channels.
...
anyhow = "1"
```
Here, we added the `gpui-kit` dependency, which brings GPUI, the platform-specific application entry point, GPUI Component, and its bundled assets together. `rust-embed` allows us to embed our own application assets into the binary, and `anyhow` is used for error handling.
...
Now, we need `Root` from GPUI Kit's component layer. Let's update our imports to:
```rust
use gpui_kit::*;
...
```
First, we are creating our application `let app = gpui_kit::application();`, then once our app is ready, `app.run(|cx| {...});` we start its event loop and this gives us an application context `cx`. After we get the context of the app, we initialise GPUI Kit and its enabled components with `gpui_kit::init(cx);`.
...
![Hello GPUI window with a fixed size](/images/gpui/first/hello-gpui-window.png)
...
![Hello GPUI with dark background and light text](/images/gpui/first/hello-gpui-colours.png)
...
![Sidebar container with background and border](/images/gpui/first/sidebar-container.png)
...
struct AppAssets;
```
The `#[folder = "assets"]` attribute tells `rust-embed` to include everything inside our `assets` directory. Now we need to make these assets available to GPUI Kit. To do that, we need to write this:
```rust
...
```
So there are two asset sources here, the first one is our own application assets `AppAssets::get(path)`. If the requested file exists in our `assets` directory, we return it; otherwise, we fall back to GPUI Kit's bundled assets with `gpui_kit::assets::Assets.load(path)`. The same pattern is used by `list()` to expose both asset sources.

Now that this is done, we need to register this asset source with the application, so change this `let app = gpui_kit::application();` to this:
```rust
let app = gpui_kit::application().with_assets(AppAssets);
```

now, if you'll run this, you should see:
![Workspace selector in the sidebar](/images/gpui/first/workspace-selector.png)
...
![Search box added below the workspace selector](/images/gpui/first/search-box.png)
...
![Navigation items in the sidebar](/images/gpui/first/navigation-items.png)
...
![Projects section with coloured dots](/images/gpui/first/projects-section.png)

The small circles are actually themselves GPUI elements:
```rust
div()
    .size(px(8.0))
    .rounded_full()
...
![User section at the bottom of the sidebar](/images/gpui/first/user-section.png)
...
![Main content header with breadcrumb and buttons](/images/gpui/first/main-header.png)
There's one thing that I want to explain: `ghost()` comes from GPUI Kit's component layer for its `Button` component. It's just a styling thing; we are not creating this style manually.
...
![Page heading for active issues](/images/gpui/first/page-heading.png)
...
![Tabs below the page heading](/images/gpui/first/tabs.png)
...
![Issue cards with priority badges](/images/gpui/first/issue-cards.png)
...
![Product issues group added](/images/gpui/first/product-issues.png)
